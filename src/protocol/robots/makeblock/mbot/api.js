...
class Api extends DefaultApi {
   ...
   * @param {!Constants.CallbackType} index_type
   * @param {Array} data
   * @param {number=} opt_data_size
   * @private
   */
  handleSensorData_(index_type, data, opt_data_size) {
    ...
    switch (index_type) {
      case Constants.CallbackType.INNER_BUTTON:
        this.dispatchSensorEvent_(index_type, Events.ButtonPressed, data[0]);
        break;
      case Constants.CallbackType.LIGHTSENSOR:
        this.dispatchSensorEvent_(index_type, Events.LightnessSensorValue,
          this.parseFloatBytes_(data));
        break;
      case Constants.CallbackType.LINEFOLLOWER:
        this.dispatchSensorEvent_(index_type, Events.LinefollowerSensorValue, {
            'left': data[3] >= 64,
            'right': data[2] >= 64,
            'raw': data,
          });
        break;
      case Constants.CallbackType.ULTRASONIC:
        this.dispatchSensorEvent_(index_type, Events.UltrasonicSensorValue,
          this.parseFloatBytes_(data));
        break;
    }
  }
